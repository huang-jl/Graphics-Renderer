...
class DiffuseLight : public Material
{
  public:
    ...
    //默认没有散射，因为是光源
    virtual bool scatter(const Ray &r_in, const Hit &rec, Vector3f &attenuation, Ray &scattered) const override
    {
        return false;
    }
    //根据材质的颜色发光
    virtual Vector3f emitted(float u, float v, Vector3f &p) const override { return emit->value(u, v, p); }
